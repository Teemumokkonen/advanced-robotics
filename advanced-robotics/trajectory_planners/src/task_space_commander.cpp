...
class trajectory_planner {
    public:
        bool init(ros::NodeHandle n) {
            ...
            pose_subs_ = n.subscribe<std_msgs::Float64MultiArray>("/elfin/impedance_controller/q", 1000, &trajectory_planner::pose_callback, this);
            twist_error_pub_ = n.advertise<geometry_msgs::Twist>("Xerr_", 1000);
            vel_pub_ = n.advertise<geometry_msgs::Twist>("/elfin/impedance_controller/command/test", 1000);
            ...
            if (!n.getParam("/elfin/impedance_controller/root_link", root_name))
            {
                ROS_ERROR("Could not find root link name");
                return false;
            }
            if (!n.getParam("/elfin/impedance_controller/tip_link", tip_name))
            {
            ...
            else
            {
                ...
                ROS_INFO("chain size %d", kdl_chain_.getNrOfJoints());
            }
            ...
            Vcmd_jnt_.data = Eigen::VectorXd::Zero(7);
            q_dot_cmd_.data = Eigen::VectorXd::Zero(n_joints_);
            fk_pos_solver_.reset(new KDL::ChainFkSolverPos_recursive(kdl_chain_));
            psd_inv_solver_.reset(new KDL::ChainIkSolverVel_pinv(kdl_chain_));
    ...
    private:
        ...
        boost::scoped_ptr<KDL::ChainIkSolverVel_pinv> psd_inv_solver_;
