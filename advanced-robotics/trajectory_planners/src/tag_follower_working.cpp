...
#include <trajectory_planners/tag_init.h>
// from kdl packages
...
class trajectory_planner {
    public:
        ...
        void target_tag_callback(const geometry_msgs::TransformStampedConstPtr &msg) {
        ...
        void calc_diff() {
            Xerr_.rot = 3.0 * diff(x_.M, xd_.M) / t_;
            Xerr_.vel = 3.0 * diff(x_.p, xd_.p) / t_;
            //Xerr_ =  1.5 * diff(x_, xd_) / t_; // error from the frame
            jnt_to_jac_solver_->JntToJac(q_, J_); // jacobian of the joint
        ...
        bool set_following(trajectory_planners::tag_init::Request  &req,
                          trajectory_planners::tag_init::Response &res) {
            is_following_tag = req.is_tag_following;
            res.result = true;
            if (req.is_tag_following == true) {
                ROS_INFO("tag following is on");
            }
            else {
                ROS_INFO("tag following is off");
            }
            return true;
        }
    ...
    private:
        ...
        ros::Subscriber target_tag_subs_;
